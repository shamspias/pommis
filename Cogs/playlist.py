    ...
    async def playlist_add(self, ctx, link):
        ...
        if playlistSize >= 25:
            return await ctx.send(
                f"{self.bot.emojiList.false} {ctx.author.mention} Your playlist (liked) is full (25 songs)!")
    ...
    async def playlist_show(self, ctx):
        ...
        if len(playlistContent) <= 0:
            return await ctx.channel.send(
                f"{self.bot.emojiList.false} {ctx.author.mention} Your playlist (liked) is empty!")
