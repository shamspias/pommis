    ...
    async def replay(self, ctx):
        ...
        if formerTrack is None:
            return await ctx.send(
                f"{self.bot.emojiList.false} {ctx.author.mention} There is no former track to replay!")
        ...
        DBQueue(self.bot.dbConnection).add(ctx.guild.id, False, requester, ctx.channel.id, trackUri, trackTitle,
                                           durationInMs, futureIndex)
