...
from discord.ext.commands import CommandOnCooldown, MissingPermissions, CommandNotFound, MissingRequiredArgument, \
    ExpectedClosingQuoteError, BotMissingPermissions
...
class EventsCog(commands.Cog, name="EventsCog"):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, CommandOnCooldown):
            ...
            if minute > 0:
                return await ctx.send(f'{ctx.author.mention} This command has a cooldown, be sure to wait for ' + str(
                    minute) + " minute(s)")
            return await ctx.send(
                f'{ctx.author.mention} This command has a cooldown, be sure to wait for {error.retry_after:.2f} second(s)')
        ...
        embed = discord.Embed(title=f"__**COMMAND ERROR**__",
                              description=f"[**GitHub**](https://github.com/shamspias/pommis)\n\n**You may report this issue on the [GitHub repository](https://github.com/shamspias/pommis)**\n```{error}```",
        ...
        if channel is not None:
            try:
                ...
                invite = None
            embed = discord.Embed(title=f"**ERROR :**", description=f"**Date :** " + datetime.datetime.now().strftime(
                "%x at %X") + f"\n**Command name :** {ctx.command.name}\n**Server link :** <{invite}>\n\n```{error}```",
                                  color=discord.Colour.red())
            embed.set_footer(
                text=f"Server : {ctx.guild.name} - {ctx.guild.id} | Author : {ctx.author} - {ctx.author.id}")
            await channel.send(embed=embed)

            print("\n\n⚠️⚠️⚠️ " + datetime.datetime.now().strftime(
                "%x at %X") + " Ignoring exception in command {}\n:".format(ctx.command), file=sys.stderr)
    ...
    async def on_message(self, message):
        ...
        if self.bot.user in message.mentions:
            await message.channel.send(
                f"{message.author.mention} My prefix on this server is : `{self.bot.command_prefix}`", delete_after=10)
